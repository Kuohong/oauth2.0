package com.power4j.framework.util;
 ...
 */
@Component("logInitialization")
public final class LoggerContextInitializer implements ServletContextAware {
     ...
     */
    protected LoggerContextInitializer() {
     ...
     */
    public LoggerContextInitializer(final String loggerContextPackageName,
        final Resource logConfigurationFile, final String logConfigurationField) {
        ...
        try {
            if (StringUtils.isNotBlank(this.loggerContextPackageName)) {
                final Collection<URL> set = ClasspathHelper
                    .forPackage(this.loggerContextPackageName);
